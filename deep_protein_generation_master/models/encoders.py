...
def fc_encoder(seqlen, latent_dim, alphabet_size=26, encoder_hidden=[250,250,250],
               encoder_dropout=[0.7,0.,0.], activation='relu', n_conditions=0):
    
    #Input layer for sequences with shape (seqlen, alphabet_size)
    x = Input(shape=(seqlen, alphabet_size,))
    h = Flatten()(x)        #Flatten sequences to a 1D vector
    
    # If conditions are provided, concatenate them to the input
    if n_conditions>0:
    ...
    z_mean=Dense(latent_dim)(h)     #Mean of the latent distribution
    z_var=Dense(latent_dim, activation='softplus')(h)       #Variance of the latent distribution
    
    # If conditions are provided, create a model with both sequences and conditions as input
    # Otherwise, create a model with only sequences as input
    if n_conditions > 0:
...
def cnn_encoder(original_dim, latent_dim,
                nchar=26, num_filters=26, kernel_size=2,
