...
class Essential_aa(EvaluationFunction):

    def __init__(self, maximize=True, worst_fitness=0):
        super(Essential_aa, self).__init__(maximize=maximize, worst_fitness=worst_fitness)

    def essential(self, candidate):
        essential = 0
        for aa in candidate:
            if aa == 'L' or aa == 'I' or aa == 'V' or aa == 'F' or aa == 'W' or aa == 'H' or aa == 'M' or aa == 'K' or aa == 'T':
                essential += 1
    ...
    def _get_fitness_batch(self, listProts):
        essential_batch =[]
        for candidate in listProts:
            essential_batch.append(self.essential(candidate))
        return essential_batch
